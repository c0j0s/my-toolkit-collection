...
def start_command_handler(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text="Available Options:", reply_markup=start_keyboard_markup)

# /gen_detail
def gen_detail_command_handler(update, context):
    context.bot.send_message(chat_id=update.effective_chat.id, text="Send me the text.")
    return GEN_DETAIL
...
def write_detail_handler(update, context):
    if update.message.text == "Yes":
        ...
        if len(details) > 0:
            update.message.reply_text("Writing {} detail to Notion...".format(len(details)))
            res = list(map(lambda x: str(controller.write_detail_object_to_notion(x)), details))
            reply = list(map(lambda x: x.title, res))
            update.message.reply_text(str(res) + " successfully written to Notion.")
        else:
...
gen_detail_handler = ConversationHandler(
    [CommandHandler('gen_detail', gen_detail_command_handler, pass_user_data=True, pass_chat_data=True)],
    {
        GEN_DETAIL: [MessageHandler(Filters.text, gen_detail_data_handler, pass_user_data=True, pass_chat_data=True)],
        WRITE_DETAIL: [MessageHandler(Filters.text, write_detail_handler, pass_user_data=True, pass_chat_data=True)]
    },
