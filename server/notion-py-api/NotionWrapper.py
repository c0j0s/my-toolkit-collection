...
class Detail(object):
    def set_title(self, nextDetailIndex):
...
class NotionWrapper(object):
    ...
    def check_if_ref_exists(self, table, keyword):
        result = self.get_tabe(table).get_rows(search=keyword)
        if len(result) > 0:
            return result[0]
        else:
            return None
    ...
    def insert_sign_in_record(self, result):
        cv = self.get_tabe('signin_record')
        row = cv.add_row()
