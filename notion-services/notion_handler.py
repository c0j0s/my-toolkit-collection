...
class NotionHandler:
    ...
    notion_table["pt_record"] = "https://www.notion.so/c0j0s/f742b24ad09046f68b383b4c70c229bc?v=ab00a48a6b124a6793f7ef7666259b15"
    ...
    def if_ref_exists(self, table, keyword):
        ...
        """
        result = self.client.get_collection_view(
            table).collection.get_rows(search=keyword)
        if len(result) > 0:
    ...
    def get_latest_detail_index(self) -> int:
        ...
        result = int(cv.build_query(sort=sort_params).execute()
                     [0].title.split(" ")[1])
    ...
    def create_vehicle_mid_type_record(self, mid, veh_type):
        ...
        veh_type_ref = self.if_ref_exists(
            self.notion_table['veh_types'], veh_type)
