...
class NotionController:
    ...
    def get_pt_schedule(self) -> str:
        """
        Compile PT schedule, return list of events
        """
        cal = Calendar()
        ...
        # get PT record
        cv = self.handler.client.get_collection_view(self.handler.notion_table["pt_record"])

        for e in cv.collection.get_rows():
            date = e.done_on
            if date is None:
                continue
            ...
            if date.end is not None:
                if not isinstance(date.end, datetime.datetime):
